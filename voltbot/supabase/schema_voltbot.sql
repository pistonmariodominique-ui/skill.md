...
CREATE TABLE public.volt_bot_status (
  ...
  cycle_lock timestamptz,   -- verrou anti-doublon volt-trader (TTL 90 s)
  monitor_lock timestamptz, -- verrou anti-doublon volt-monitor (TTL 100 s)
...
CREATE TABLE public.volt_settings (
  ...
  instruments text[] DEFAULT ARRAY['GOLD'::text, 'SILVER'::text, 'NATURALGAS'::text],
  ...
  atr_sl_mult numeric DEFAULT 1.5,       -- SL bot = 1.5 × ATR(14, M15)
  atr_tp_mult numeric DEFAULT 3.0,       -- TP bot = 3.0 × ATR → R:R 1:2
...
CREATE TABLE public.volt_trades (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  instrument text CHECK (instrument = ANY (ARRAY['GOLD'::text, 'SILVER'::text, 'NATURALGAS'::text])),
  ...
  sl_price numeric,                      -- SL du BOT (serré, ATR) — le stop garanti broker est plus large
  tp_price numeric,                      -- TP du BOT
